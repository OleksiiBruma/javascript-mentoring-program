...
fastify.post("/api/login", async (request, reply) => {
  ...
  if (user) {
    ...
    reply.code(401).send({ message: "incorrect login or password" });
  }
});
...
fastify.put("/api/courses/:id", async (request, reply) => {
  const findChain = db.get("courses").find({ id: request.params.id });

  if (findChain.value()) {
    return findChain.assign(request.body).write();
  } else {
    reply.code(404).send({ message: "not found" });
  }
});
